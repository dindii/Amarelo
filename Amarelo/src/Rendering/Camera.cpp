...
namespace Amrl
{
	...
	void Camera::RenderParallel(const HittableList* world, uint32_t threadIdx)
	{
		uint32_t widthPortion = m_Width / m_NumThreads;
		
		uint32_t startFromWidth = widthPortion * threadIdx;
		uint32_t endFromWidth = startFromWidth + widthPortion;
		
		//slicing the scene horizontally is more cache friendly but usually our images have way more width than height
		//for not too large images, slicing it horizontally is more performant but for really long images, we can just use more threads and each one will have a larger area to work with. 
		//accordingly to my measurements, this ends up being more performant for larger images.
		for (int32_t y = m_Height - 1; y > 0; y--)
		{
			for (uint32_t x = widthPortion * threadIdx; x < widthPortion * (threadIdx + 1); x++)
			{
				vec3<float> color = 0;
				...
				m_Framebuffer->SetPixel(x, y, color  * m_SPPScale);
			}
		}
	}
	...
	void Camera::Render(const HittableList* world) const
	{
		...
		std::vector<std::thread> rayColorWorkerThreads(m_NumThreads);
	
		for (uint32_t i = 0; i < m_NumThreads; i++)
			rayColorWorkerThreads[i] = std::thread(&Camera::RenderParallel, *this, world, i);
	
	
		for (uint32_t i = 0; i < m_NumThreads; i++)
			rayColorWorkerThreads[i].join();
	
		std::chrono::duration now = std::chrono::system_clock::now().time_since_epoch();
		time_t actualTime = std::chrono::duration_cast<std::chrono::seconds>(now - before).count();
	
		m_Framebuffer->Flush();
	
		std::cout << "\nDone! Time taken: " << actualTime << " second(s)" << std::endl;
	}
	...
	Ray Camera::GenRay(int32_t x, int32_t y) const
	{
		...
		vec3<float> rayOrigin = (m_DefocusAngle <= 0) ? m_CameraPosition : DefocusDiskSample();
	...
	vec3<float> Camera::DefocusDiskSample() const
	{
		vec3<float> randomDiskSample = Amrl::RandomInUnitDisk();
		return m_CameraPosition + (randomDiskSample.x * m_DefocusDiskU) + (randomDiskSample.y * m_DefocusDiskV);
	}
}